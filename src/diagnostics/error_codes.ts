...
export const enum ErrorCode {
  ...
  EFF_INFER_UNKNOWN_BUILTIN = "E212",
...
export const ERROR_MESSAGES: Record<ErrorCode, string> = {
  ...
  [ErrorCode.DECIMAL_DOUBLE_MIXING]: "Cannot combine Decimal and Double in '{operator}'. Double is binary floating-point and would inject rounding error into an exact Decimal. Use Decimal literals (e.g. 1.08m) on both sides, or Int/Long (exact promotion).",
  ...
  [ErrorCode.EFF_INFER_UNKNOWN_BUILTIN]: "Effect of builtin '{builtin}' called by '{func}' is unknown; it is NOT assumed pure",
  ...
  [ErrorCode.PII_ANALYZER_FAILED]: "PII safety analysis failed for this module — the editor cannot verify whether sensitive data is correctly handled. This policy should not be deployed until the analysis succeeds. Internal reason: {reason}",
...
export const ERROR_METADATA: Record<ErrorCode, ErrorMetadata> = {
  ...
  [ErrorCode.DECIMAL_DOUBLE_MIXING]: { code: ErrorCode.DECIMAL_DOUBLE_MIXING, category: 'type', severity: 'error', message: "Cannot combine Decimal and Double in '{operator}'. Double is binary floating-point and would inject rounding error into an exact Decimal. Use Decimal literals (e.g. 1.08m) on both sides, or Int/Long (exact promotion).", help: "Make both operands Decimal (suffix m), or use Int/Long which promote exactly to Decimal." },
  ...
  // P0-R2 (codex review Medium #10): PII codes 统一分类为 'pii'（原本是
  // 'type' 是历史遗留，让 PII_CODES 自动派生不可能正确）
  [ErrorCode.PII_ASSIGN_DOWNGRADE]: { code: ErrorCode.PII_ASSIGN_DOWNGRADE, category: 'pii', severity: 'error', message: "Cannot assign PII data to lower-level target: {source} -> {target}", help: "Use a sanitization function or declare a matching @pii level on the target." },
  [ErrorCode.PII_SINK_UNSANITIZED]: { code: ErrorCode.PII_SINK_UNSANITIZED, category: 'pii', severity: 'error', message: "PII level {level} data output to {sinkKind} without sanitization", help: "Call redact() or tokenize() before output to reduce sensitivity." },
  ...
  [ErrorCode.EFF_INFER_UNKNOWN_BUILTIN]: { code: ErrorCode.EFF_INFER_UNKNOWN_BUILTIN, category: 'effect', severity: 'warning', message: "Effect of builtin '{builtin}' called by '{func}' is unknown; it is NOT assumed pure", help: "Declare it via ASTER_EFFECT_CONFIG prefixes, supply an imported effect signature, or use a known stdlib namespace." },
  ...
  [ErrorCode.PII_ANALYZER_FAILED]: { code: ErrorCode.PII_ANALYZER_FAILED, category: 'pii', severity: 'error', message: "PII safety analysis failed for this module — the editor cannot verify whether sensitive data is correctly handled. This policy should not be deployed until the analysis succeeds. Internal reason: {reason}", help: "Try saving and reloading the file. If the error persists, contact your administrator or report this issue with the source code attached." },
  ...
  [ErrorCode.PII_IMPLICIT_UPLEVEL]: { code: ErrorCode.PII_IMPLICIT_UPLEVEL, category: 'pii', severity: 'warning', message: "Implicit PII level escalation detected: {source} -> {target}", help: "Add explicit type annotations for level changes to aid auditing." },
  [ErrorCode.PII_SINK_UNKNOWN]: { code: ErrorCode.PII_SINK_UNKNOWN, category: 'pii', severity: 'warning', message: "PII data may flow to {sinkKind} without annotation", help: "Add @pii annotation to track sensitive data flow." },
