/**
 * Browser-compatible effect inference module.
 *
 * This module provides effect inference without Node.js dependencies (no file system access).
 * It uses hardcoded default prefixes and works in browser/edge runtime environments.
 *
 * For full effect inference with custom configuration, use the main effect_inference module.
 */
...
/**
 * Infer effects for a Core module (browser-compatible).
 *
 * This version uses hardcoded default IO/CPU prefixes and doesn't require
...
export function inferEffects(core: Core.Module, options?: EffectInferenceOptions): TypecheckDiagnostic[] {
  ...
  // First pass: collect local effects and constraints
  ...
  // Second pass: add callee declared effects to requiredEffects
  ...
  // Note: In browser mode, we skip caching effect signatures since there's no module cache
...
function topologicalSort(
  ...
  while (queue.length > 0) {
    ...
    visited[idx] = true;
    order.push(idx);
    const edges = componentEdges.get(idx);
    if (!edges) continue;
